...
// --- https://github.com/SamJUK/TinderAccessToken/blob/master/index.js ---
(async () => {
  // Check that we have credentails supplied otherwise die
  const FB_USERNAME =
    process.env.FACEBOOK_LOGIN ||
    (() => {
      console.log("Missing ENV FB_USERNAME");
      process.exit(1);
    })();
  const FB_PASSWORD =
    process.env.FACEBOOK_PASS ||
    (() => {
      console.log("Missing ENV FB_PASSWORD");
      process.exit(1);
    })();
  ...
  const is_login_page = (await page.title()).toLowerCase().includes("anmelden");
  if (is_login_page) {
    console.log("Authenticating with facebook");

    await page.evaluate(
      (FB_USER, FB_PASS) => {
        document.querySelector("#m_login_email").value = FB_USER;
        document.querySelector("#m_login_password").value = FB_PASS;
      },
      FB_USERNAME,
      FB_PASSWORD
    );
    ...
    const is_still_login_page = (await page.title())
      .toLowerCase()
      .includes("anmelden");
    ...
    // Check for 2 Factor
    const is_2fa_page = false; //await page.evaluate(() => document.querySelector('input[name="approvals_code"]')) !== null;
    if (is_2fa_page) {
      ...
      await Promise.race([
        ...
        const is_auth_page = await page.evaluate(TINDER_APP_ID => {
          document.querySelector('input[name="app_id"]') !== null &&
            document.querySelector('input[name="app_id"]').value ===
              TINDER_APP_ID;
        }, TINDER_APP_ID);
  ...
  const is_auth_page = true; //await page.evaluate(() => document.querySelector('input[name="app_id"]').value) === TINDER_APP_ID;
  if (is_auth_page) {
    ...
    page.on("response", response => {
      ...
      response
        ...
        .then(function(textBody) {
          ...
          const access_token = new RegExp(".+&access_token=(.+?)&").exec(
            textBody
          )[1];
