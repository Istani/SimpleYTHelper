...
export default async function ConnectionsPage({ searchParams }) {
  ...
  const [accounts, discordConfiguration] = prisma ? await Promise.all([
    ...
    prisma.oauthProviderConfiguration.findUnique({ where: { provider: 'discord' }, select: { isActive: true } }).catch(() => null),
  ]) : [[], null];
  ...
  return <DashboardShell user={user} label="Konto" title="Verknüpfte Dienste">
    ...
    <section className="panel">
      ...
      {discordConfiguration?.isActive ? (discordAccount ? <a className="button button-secondary" href="/api/connections/discord/start">Erneut verbinden</a> : <a className="button button-primary" href="/api/connections/discord/start">Mit Discord verbinden</a>) : <p className="form-hint">Discord-Verknüpfungen werden gerade vorbereitet. Bitte versuche es später erneut.</p>}
    </section>
