...
export default async function DiscordModerationPage() {
  ...
  if (prisma) try { [guilds, dms, guildObservations] = await Promise.all([
    prisma.discordGuild.findMany({ orderBy: { name: 'asc' }, include: { _count: { select: { messages: true } }, members: { where: { user: { isBot: true } }, include: { user: { select: { username: true, globalName: true } } } } } }),
  ...
  return <DashboardShell user={user} label="Discord · Moderation" title="Konversationen im Kontext prüfen."><div className="moderation-grid"><section className="panel moderation-list moderation-server-panel"><SourceList panel={false} title="Server mit Bots" description="Server mit gespeicherten Bot-Mitgliedern; aufgeführt sind ihre Discord-Anzeigenamen." records={guildGroups.withBots} kind="guild" /><SourceList panel={false} title="Nur Selfbots" description="Server ohne bekannte Bot-Mitglieder; sichtbar über die aufgeführten Selfbot-Konten." records={guildGroups.selfbotOnly} kind="guild" /></section><SourceList title="Direkte Nachrichten" description="DM- und Gruppen-DM-Channels mit zuletzt bekannten Schreibenden und Profilbild." records={dms} kind="dm" /></div></DashboardShell>;
}
