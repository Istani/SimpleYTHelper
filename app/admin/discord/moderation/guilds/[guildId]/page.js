...
export default async function GuildModerationPage({ params }) {
  ...
  if (!guild) notFound();
  const channels = sortDiscordGuildChannels(guild.channels);
  return <DashboardShell user={user} label="Discord · Moderation" title={guild.name}>
    ...
    <div className="detail-grid">
      ...
      <section className="panel detail-panel"><div className="panel-title"><span className="kicker">Channels</span><h2>Bekannte Channels</h2></div><div className="table-wrap"><table><thead><tr><th>Name</th><th>Typ</th><th>Nachrichten</th><th>Status</th><th></th></tr></thead><tbody>{channels.map((channel) => { const type = describeDiscordChannel(channel); return <tr className={channel.deletedAt ? 'history-row' : ''} key={channel.id}><td><span className="discord-channel-name" style={{ paddingInlineStart: `${channel.depth * 1.25}rem` }}>{type.icon} {channel.name || 'Unbenannter Channel'}{type.isThread && channel.parentName ? <small className="discord-channel-parent">Thread zu #{channel.parentName}</small> : null}</span></td><td>{type.label}</td><td>{channel._count.messages}</td><td><HistoryBadge deletedAt={channel.deletedAt} /></td><td><Link className="text-link" href={`/admin/discord/moderation/channels/${encodeURIComponent(channel.id)}`}>Nachrichten ›</Link></td></tr>; })}</tbody></table></div></section>
