...
export default async function ChannelDetailPage({ params }) {
  ...
  const channel = prisma ? await prisma.discordChannel.findUnique({ where: { id: channelId }, include: { guild: true, messages: { orderBy: { createdAt: 'desc' }, take: 100, include: { author: true, media: { orderBy: [{ kind: 'asc' }, { position: 'asc' }] } } } } }).catch(() => null) : null;
  if (!channel) notFound();
