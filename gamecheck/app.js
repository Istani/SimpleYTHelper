...
async function get_games() {
  ...
  for (var i = 0; i < sales.length; i++) {
    ...
    var active = await Check.query()
      .where("category", tmp_obj.category)
      .where("game", tmp_obj.game);
    if (active.length > 0) {
    ...
    tmp_tweet.message =
      "" + tmp_obj.display_title + " " + tmp_obj.link + " #werbung";
  ...
  await Game.query()
    .delete()
    .where("updated_at", "<", Check.DeleteDate);
  await Check.query()
    .delete()
    .where("updated_at", "<", Check.DeleteDate);
  await Merch.query()
