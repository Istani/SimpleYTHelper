...
function LoadButton({ section, load, loading, hasMore }) {
  return <button className="button button-secondary" type="button" disabled={loading} onClick={() => load(section)}>{loading ? 'Lädt …' : hasMore ? `Weitere ${sectionLabels[section]} laden` : `${sectionLabels[section]} laden`}</button>;
}
