...
function NewBotDialog() {
  ...
  return <Dialog title="Discord-Instanz registrieren" triggerLabel="Instanz hinzufügen" primary><form action={action} className="bot-form dialog-form"><label>Interne Instanz-ID<input name="botId" required minLength="3" maxLength="64" pattern="[A-Za-z0-9][A-Za-z0-9_-]{2,63}" placeholder="community-reporter" /></label><label>Runtime-Typ<select name="accountKind" defaultValue="bot"><option value="bot">Discord-Bot</option><option value="selfbot">Discord-Selfbot</option></select></label><label>Discord Token<input name="token" type="password" required minLength="20" autoComplete="new-password" /></label><CapabilityControls /><p className="form-hint">Der Token wird nie angezeigt, protokolliert oder im Browser wieder ausgegeben. Selfbots laufen isoliert und erfassen in diesem Stand ausschließlich eingehende Discord-Daten.</p><Notice state={state} /><button className="button button-primary" disabled={pending}>{pending ? 'Speichert …' : 'Instanz registrieren'}</button></form></Dialog>;
}
