...
const url = "https://metagamerscore.com/achievement?utf8=%E2%9C%93&user=9738&sort=myach_earned";
  ...
  await requestp({ url: url }, async function (error, response, body) {
    ...
    for (var col = 0; col < archivment_table[0].length; col++) {
      for (var row = 0; row < archivment_table.length; row++) {
        if ((typeof dump[col]) == "undefined") { dump[col] = []; }
        if (typeof dump[col][row] == "undefined") { dump[col][row] = ''; }
    ...
    for (var i = dump.length - 1; i > 0; i--) {
      ...
      if (dat > settings.last_time) {
        ...
        tmp_tweet.message = 'New Achievement: "' + dump[i][0] + '" in "' + dump[i][3] + '"';
        if (details.length > 0) {
