...
async.parallel(
  [
    function(callback) {
      db.query(
        "SELECT * FROM simpleyth_oauth_secrets WHERE service = 'youtube'",
        function(err, result) {
          if (err) {
            callback(err);
            return;
    ...
    function(callback) {
      db.query(
        "SELECT * FROM simpleyth_oauth_botcredentials WHERE service = 'youtube'",
        function(err, result) {
          if (err) {
            callback(err);
            return;
  ...
  function(err) {
    ...
    db.query(
      "UPDATE simpleyth_login_token SET cronjob=false WHERE service = 'youtube'",
      function(err, result) {
        // Yeay
        if (err) {
          console.error(err);
...
function Gen_New_Cronjobs() {
  db.query(
    "SELECT * FROM simpleyth_login_token WHERE cronjob=false AND service = 'youtube'",
    function(err, result) {
      if (err) {
        console.error(err);
      } else {
        if (result.length > 0) {
          db.query(
            "UPDATE simpleyth_login_token SET cronjob=true WHERE id=? ",
            [result[0].id],
...
function Refresh_Token(token_data, org_function) {
  ...
  request(
    ...
    function(err, res) {
      ...
      var temp_sql = db.format(
        "UPDATE simpleyth_login_token SET access_token=? WHERE id=?",
        [token_data.access_token, token_data.id]
      );
...
function GetChannel(token_data) {
  async.parallel(
    [
      function(callback) {
        db.query(
          "SELECT * FROM simpleyth_login_token WHERE id=?",
          [token_data.id],
          function(err, result) {
            if (err) {
              console.error(err);
        ...
        function(err, res) {
          ...
          try {
            ...
            if (typeof data.items[0].snippet.thumbnails.high !== "undefined") {
              ...
              typeof data.items[0].snippets.thumbnail.medium !== "undefined"
            ) {
            ...
            channel.import_details(
              null,
              err => {
                if (err) {
                  console.error("Channel Import", err);
                }
              },
              channel_details
            );
