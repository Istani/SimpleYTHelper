...
if ($tt["last_used"]+$tt["cooldown"]<time()) {
  ...
  if ($tt["token"] == "null") {
    ...
    $listRequests = $database->sql_select("videos_snippet","videoid", "`channelid`='".$_SESSION['user']['youtube_user']."' AND `ignore`=0 ORDER BY last_contentDetailsupdate LIMIT ".$req_count, true);
  } else {
  ...
  for($i=0;$i<count($data4sql);$i++) {
    ...
    if (isset($listResponse["items"][0])) {
      ...
      $tmp_row4sql["videoId"]= protected_settings($row4sql[$i]["videoId"]);
      $row4sql=null;
      $row4sql=$tmp_row4sql;
