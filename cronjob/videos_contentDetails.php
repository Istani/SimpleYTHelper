...
if ($tt["last_used"]+$tt["cooldown"]<time()) {
  ...
  if ($tt["token"] == "null") {
    ...
    $listRequests = $database->sql_select("videos_snippet","videoId", "`channelid`='".$_SESSION['token']['channel_id']."' AND `ignore`=0 ORDER BY last_contentDetailsupdate LIMIT ".$req_count, true);
  } else {
