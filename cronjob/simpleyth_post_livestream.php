...
if (true) {
  ...
  $videos=$database->sql_select("youtube_livestream", "*", "youtube_snippet_channelid='".$_SESSION['user']['youtube_user']."' AND (youtube_snippet_actualendtime IS NULL OR youtube_snippet_actualendtime='') ORDER BY youtube_snippet_actualstarttime DESC LIMIT 1",false);
  if (count($videos)>0) {
    if ($videos[0]["simple_lastupdate"]<time()-30*60) {
      $videos[0]['youtube_snippet_actualendtime']="ERROR";
      $database->sql_insert_update("youtube_livestream", $videos[0]);
    } else {
      $livestream['id']=$videos[0]['youtube_id'];
      $livestream['message']="!yt livestream";
      $livestream['chatroom']=$videos[0]['youtube_snippet_livechatid'];
      $ad_params=explode("|", $videos[0]['youtube_snippet_title']);
      $livestream['game']=$ad_params[0];
    }
  }
}
...
if (true) {
  ...
  if (count($videos)>0) {
    $livestream['id']=$videos[0]['twitch_id'];
    $livestream['message']="!twitch livestream";
  }
}
// NOTE: Ggf weitere Dienste


if ($livestream['id']!='null' && $livestream['id']!=$tt["token"]) {
    ...
    foreach ($the_hosts as $t_host => $t_channel) {
      if ($t_channel!="0") {
        ...
        if ($t_user!="") {
          ...
          $add_post['message']=$livestream['message'];
          $add_post['process']=0;
  ...
  if ($livestream['game']!='') {
    ...
    Generate_Amazon_Ad($amazon, $database, $livestream['game'], $_SESSION['user']['email'], false, true);
  }
  
}
$tt["token"]=$livestream['id'];
